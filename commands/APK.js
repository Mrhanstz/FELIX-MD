...
const { dBinary, eBinary } = require("../hanstz/binary");
const fancy = require("../hanstz/style");
...
hans({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please insert the image URL and FELIX-MD will extract the text for you.");
    }
...
hans({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please describe your image and FELIX-MD will generate it.");
    }
    const prompt = args.join(" ");
  ...
  async (origineMessage, zk, commandeOptions) => {
    ...
    try {
      ...
      if (data?.status && data?.result?.url) {
        ...
        const caption = "*POWERED BY FELIX-MD*";
...
hans({
  ...
  if (!githubUsername) {
    return respond("Give me a valid GitHub username like: github HansTz");
  }
...
hans({
  ...
  try {
    ...
    try {
      ...
      const caption = "Downloaded by FELIX Md";
